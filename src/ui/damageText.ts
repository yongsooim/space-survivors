...
class DamageTextPool {
  ...
  constructor() {
    for (let i = 0; i < consts.numberOfDamageText; i++) {
      let temp = new PIXI.Text('', { fontFamily: 'Consolas', fontSize: 20, fill: 0xffffff, align: 'center', dropShadow: true, strokeThickness: 2 });
      temp.scale.set(0.09);
      temp.anchor.set(0.5);
      this.pool.push(temp);
    }
    ...
    for (let i = 0; i < consts.numberOfDamageText; i++) {
      let temp = new PIXI.Text('', { fontFamily: 'Consolas', fontSize: 20, fill: 0xff0000, align: 'center', dropShadow: true, strokeThickness: 2 });
      temp.scale.set(0.09);
      temp.anchor.set(0.5);
  ...
  playerhit(x: number, y: number, damage: number) {
    ...
    let hitTextAnimation = (delta: number) => {
      this.hitpool[currentCursor].alpha -= 0.01 * delta;
      let speed = this.hitpool[currentCursor].alpha * this.hitpool[currentCursor].alpha;
