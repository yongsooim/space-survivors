...
import { autoAttack1Filter, Isa } from './worker1'

export declare class AutoAttack1Pool { // for alias
  cursor: number
  pool: Box2D.b2Body[]
  ptrToIdx: number[]
  updateSabPosition(): void
  disableByPtr(ptr: number): number
  fire(): void
}

export const createAutoAttack1Pool = (box2D: typeof Box2D & EmscriptenModule, world: Box2D.b2World, autoAttack1filter: Box2D.b2Filter, sa: Isa) => {
  ...
  class AutoAttack1Pool {
    ...
    fire() {
      ...
      sa.autoAttack1Enabled[this.cursor] = 1;
